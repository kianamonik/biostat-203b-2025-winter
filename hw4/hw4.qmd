...
labevents_tble <- labevents_tble |>
  inner_join(icustays_tble |> select(subject_id, stay_id, intime), by = "subject_id")
...
chartevents_tble <- chartevents_tble |>
  rename(
    ...
    blood_pressure_systolic = vital_220179,
    blood_pressure_diastolic = vital_220180,
    temperature_f = vital_223761,
...
icustays_filtered <- icustays_tble |>
  ...
  inner_join(
    patients_tble |> select(subject_id, anchor_age, anchor_year, anchor_year_group, dod, gender),
    by = "subject_id"
  ) |>
...
admissions_selected <- admissions_tble |>
  select(subject_id, hadm_id, race, insurance, admission_type, admission_location, discharge_location) |>
  collect()
