...
chartevents <- read_parquet("chartevents_filtered.parquet")
```
...
ggplot(chartevents_with_labels, aes(x = charttime, y = valuenum, color = abbreviation)) +
  geom_point(size = 1.2) + 
  geom_line(size = 0.8) +  
  ...
  scale_x_datetime(
    breaks = seq(
      floor_date(min(chartevents_with_labels$charttime, na.rm = TRUE), unit = "6 hours"),
      ceiling_date(max(chartevents_with_labels$charttime, na.rm = TRUE), unit = "6 hours"),
      by = "6 hours"
    ),
    date_labels = "%b %d %H:%M" 
  ) +
...
```{r, eval=FALSE}
chartevents_icu <- chartevents %>%
  filter(itemid %in% c(220045, 220179, 220180, 223761, 220210)) %>% 
  left_join(
    select(icustays_tble, subject_id, stay_id, intime),
    by = "subject_id"
  ) %>%
  mutate(stay_id = coalesce(stay_id.x, stay_id.y)) %>% 
  select(-stay_id.x, -stay_id.y)
...
chartevents_tble <- chartevents_icu_second %>%
...
cat("Number of rows in final table:", nrow(chartevents_tble), "\n")
```
